...
function drawLineWithLabels(data,color){
    ...
    for(let i=0; i<data.length; i++){
        ...
        //DIBUJAR LA LINEA
        if (i=== 0){
            ctx.moveTo(x,y); //UBICA POSICIÓN DEL PUNTERO
        }else{
            ctx.lineTo(x,y); //TRAZAR LINEA A POSICIÓN CALCULADA EN "X" e "Y"
        }
...
function drawAxes(){
    ...
    for(let i=0; i< labels.length; i++){ //SE RECORRE EL ARREGLO DE ETIQUETAS
        const x = (i/(labels.length - 1))*(canvas.width - 100) + 50; //LA ETIQUETA ESTARÁ EN LA MISMA POSICIÓN EL PUNTERO GRAFICADO
    ...
    //ETIQUETA EN EJE Y
    for(let i=20; i<=40; i+=5){ //EN EL EJE Y SE MOSTRARÁN RANGOS DESDE LOS 20C HASTA LOS 40C CON SALTOS DE 5C
        const y = canvas.height - 50 - (i-20) * 10; //POSICIÓN DE CADA GRADO CENTIGRADO, IRÁ DECREMENTANDO
        ctx.fillText(i+"C", 20, y+5); //DIBUJAR
    }
}

drawAxes();
drawLineWithLabels(sanSalvador, 'red'); //LINEA DE COLOR ROJO PARA LA GRÁFICA DE SAN SALVADOR
drawLineWithLabels(santaTecla,'blue'); //LÍNEA DE COLOR AZUL PARA LA GRÁFICA DE SANTA TECLA

//LEYENDA DE LA GRÁFICA
ctx.fillStyle = 'red';
ctx.fillRect(70,20,10,10); //DIBUJA UN RECTÁNGULO DE 70px DE ANCHO, 20px DE ALTO, POSICIÓN X=10, POSICIÓN Y=10
ctx.fillStyle = 'black';
ctx.fillText("San Salvador",85,30); //COLOCAR EL TEXTO: "SAN SALVADOR" EN LA POSICIÓN EJE X=85, EJE Y=30

ctx.fillStyle = 'blue';
ctx.fillRect(170,20,10,10); //DIBUJA UN RECTÁNGULO DE 170px DE ANCHO, 20px DE ALTO, POSICIÓN X=10, POSICIÓN Y=10
ctx.fillStyle = 'black';
